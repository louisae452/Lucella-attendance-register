...
from .models import DailyRegister, Email, Student, Subject, Timetable
...
class TestParentForm(TestCase):
    ...
    def test_form_is_not_validated_name(self):
        """
        Tests ParentForm is not validated if parent_name is not a user
        in group parent
        """
        data = {
...
class TestTeacherForm(TestCase):
    ...
    def setUp(self):
        ...
        self.test_user = User. objects.create(
            username='MargaretMillicent', password='testpassword')
    ...
    def test_form_is_not_validated_name(self):
        """
        Tests TesacherForm is not validated if teacher_name is not a user
        in group teacher
        """
        data = {
...
class TestRegisterForm(TestCase):
    ...
    def test_form_is_not_valid_code(self):
        """
        Tests RegisterForm is not validated if student_code is not initial
        value
        """
        data = {
            'student_code': self.test_student.pk,
...
class TestAbsenceForm(TestCase):
    ...
    def test_fom_is_not_valid_date(self):
        """
        Tests AbsenceForm is not validated if the date is not entered in
        the correct format
        """
        data = {
            'date': '19-04-2026',
...
class TestGivereasonForm(TestCase):
    ...
    def setUp(self):
        """
        Creates student, teacher, subject, timetable aned dailyregister
        instances.
        """
        parent_group, _ = AuthGroup.objects.get_or_create(name='parent')
        self.test_user = User. objects.create(
        ...
        self.test_student = Student.objects.create(
            ...
            parent_name=self.test_user)
        teacher_group, _ = AuthGroup.objects.get_or_create(name='teacher')
        self.test_teacheruser = User. objects.create(
            username='MargaretMillicent', password='testpassword')
        self.test_teacheruser.groups.add(teacher_group)
        self.test_subject = Subject.objects.create(
            subject_name='Maths A',
            teacher_name=self.test_teacheruser,
            set=1,
            room=1)
        ...
        self.test_register = DailyRegister.objects.create(
...
class TestPendingabsenceForm(TestCase):
    ...
    def setUp(self):
        """
        Creates student, teacher, subject, timetable and dailyregister
        instances.
        """
        parent_group, _ = AuthGroup.objects.get_or_create(name='parent')
        self.test_user = User. objects.create(
    ...
    def test_form_is_valid(self):
        ...
        pendingabsence_form = PendingabsenceForm(
            data, instance=self.test_register)
...
class TestGetclassForm(TestCase):
    ...
    def test_form_is_not_valid(self):
        """
        Tests GetclassForm is not validated when an invalid instance
        is enterered
        """
        data = {
            'subject_name': "",
...
class TestRemoveForm(TestCase):
    """
    Tests RemoveForm. Requires instance of registered and deregistered
    students.
    """
    def setUp(self):
        """
        Creates instances of registered and deregisterd students.
        Requires parent.
        """
        parent_group, _ = AuthGroup.objects.get_or_create(name='parent')
        self.test_user = User. objects.create(username='FlorenceFox', password='testpassword')
    ...
    def test_form_is_not_valid(self):
        """
        Tests RemoveForm is not validated when a deregistered student
        selected
        """
        data = {
