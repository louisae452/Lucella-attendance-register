...
def add_parent(request):
    ...
    **Template:**
    ...
    if not (request.user.is_authenticated and request.user.groups.filter(
            name='admissions_officer').exists()):
        raise PermissionDenied(
            "Only the Admissions Officer has access to this page")
...
def add_parentdata(request):
    ...
    Displays an individual instance on :model:`attendance.Parent`
    **Context**
    ``parentform``
        An instance of :form:`attendance.ParentForm``
    **Template:**
...
def add_student(request):
    """
    Adds a new student
    Displays an instance of :model:`attendance.Student`
    **Context**
    ``studentform``
         An instance of :form:`attendance.StudentForm``
    **Template:**
...
def add_teacher(request):
    ...
    **Template:**
    ...
    if not (request.user.is_authenticated and request.user.groups.filter(
            name='admissions_officer').exists()):
        raise PermissionDenied(
            "Only the Admissins Officer has access to this page")
    if request.method == "POST":
        userform = UserForm(data=request.POST)
        if userform.is_valid():
            newuser = userform.save()
...
def add_teacherdata(request):
    ...
    Displays an instance of :model:`attendance.Teacher`
    **Context**
    ``teacherform``
...
def student_detail(request, student_code):
    ...
    **Context**
        ``student``
            an instance of :model:`attendance.Student`
        ``student_page'
            queryset of all the :model:`attendance.DailyRegister` records
            of the student that fit on a page,
        **Template**
...
def sendemail(request, student_code):
    ...
    Dispalys an instance of :model:`attendance.Sentemail`
    **Context**
    ``userform``
    ...
    if request.method == "POST":
        ...
        if getregisterform.is_valid():
            ...
            if getregisterform.cleaned_data['day'] == weekday:
                ...
                currentsessionid = get_object_or_404(
                    Timetable,
                    day=getregisterform.cleaned_data['day'],
                    session=getregisterform.cleaned_data['session'],
                    subject_name=getregisterform.cleaned_data['subject_name']
                    )
                # Find the set for the current session
                currentset = get_object_or_404(
                    Subject,
                    subject_name=getregisterform.cleaned_data['subject_name']
                    ).set
                ...
                if currentset == 0 or currentset == 1:
                    students = Student.objects.filter(
                        group=currentset, deregistered=False)
                elif currentset == 2 or currentset == 3:
                    students = Student.objects.filter(
                        sex=currentset, deregistered=False)
...
def saveregister(request):
    ...
    Displays instances of :model:`attendance.DailyRegister`
    **Context**
    ``date``
        Today's date
    ``sessionid``
...
def pending_absences(request):
    ...
    **Context**
    ``pending``
         queryset of all pending instances in :model:`attendnace.DailyRegister`
    **Template:**
...
def absence_detail(request, student_code, date, session_id):
    """
    Allows to review and individual pending instance of
    :model:`attendance.DailyRegister`
    Sends email to parent when absence is deemed unauthorised
    **Context**
    ``student``
        An instance of :model:`attendance.Student`
    ``absence``
        An instance of :model:`attendance.DailyRegister`
    ``review``
    An instance of |:form:`attendance.PendingabsenceForm
    **Template:**
    :template:`attendance/absence_detail.html`
    """
    if not (request.user.is_authenticated and request.user.groups.filter(
            name='attendance_officer').exists()):
        raise PermissionDenied(
    ...
    absence = get_object_or_404(
        DailyRegister,
        student_code__student_code=student_code,
    ...
    if request.method == "POST":
        ...
        if absenceform.is_valid():
            ...
            if absenceform.instance.status == 3:
                ...
                newemail = Sentemail.objects.create(student_code=student,
                                                    subject=email3)
                ...
                html_content = render_to_string(
                    'attendance/subject3_email.html', text_info)
                text_content = strip_tags(html_content)
                message = EmailMultiAlternatives(
                    subject,
                    text_content,
...
def truanting_list(request):
    """
    Displays a list of all instances of :model:`attendance.Student`
    that are truanting today
    Sends an email to the parent of each student on the list
    **Context**
    ``truantinglist``
            a queryset of :model:`atendance.DailyRegister`
    ``today``
        today's date
    **Template:**
    ...
    if not (request.user.is_authenticated and request.user.groups.filter(
        ...
        raise PermissionDenied(
            "Only the Attendance Officer has access to this page")
    today = date.today()
    truantinglist = DailyRegister.objects.filter(
        date=today, mark=1, reason_for_absence='')
    if request.method == "POST":
        # get the email.
        ...
        with transaction.atomic():
            for record in truantinglist:
                ...
                parentmail = parent.email
                newemail = Sentemail.objects.create(
                    student_code=student, subject=email2)
                subject = newemail.subject
                # Write the email
                text_info = {
                    'studentname': student.student_name,
...
def get_class(request):
    """
    Displays a list of all instances of:model:`attendance.Student` in a
    specific class
    **Context**
    ``classlist``
    ...
    if request.method == "POST":
        ...
        if classlist.is_valid():
            ...
            if subject.set == 0 or subject.set == 1:
                ...
                students = Student.objects.filter(
                    sex=subject.set, deregistered=False)
            elif subject.set == 4 or subject.set == 5:
                students = Student.objects.filter(
                    music_option=subject.set, deregistered=False)
            # get appropriate session values
            sessionids = Timetable.objects.filter(
                subject_name__subject_name=group).order_by().values_list(
            ...
            students = students.annotate(
                ...
                present_sessions=Count(
                    'dailyregister',
                    filter=Q(
                        dailyregister__session_id__in=sessionids,
                        dailyregister__mark=0)),
            )
            for student in students:
                if student.total_sessions != 0:
                    student.attendance = round(
                        (student.present_sessions/student.total_sessions)*100,
                        2)
                else:
...
def class_detail(request, subject_name, student_code):
    """
    Displays instances of :model:`attendance.DailyRegister`
    of an individual student in an individual subjext
    **Context**
    ``sessionlist``
        queryset of :model:`attendane.DailyRegister`
    ``subjectname``
        name of the subject
    **Template:**
    ...
    """
    sessionids = Timetable.objects.filter(
        subject_name__subject_name=subject_name).values_list(
            'id', flat=True).distinct()
    sessionslist = DailyRegister.objects.filter(
        student_code__student_code=student_code, session_id__in=sessionids)
    return render(
        request,
...
def children_list(request):
    """
    Displays a list of all the instances of :model:`attendance.Student`
    registered belonging to a parent user.
    **Context**
    ``children``
        Queryset of the :model:`attendance.Student` registered instances
        of children belonging to the parent user.
    **Template:**
    :template:`attendance/landing.html`
    """
    children = Student.objects.filter(
        parent_name=request.user, deregistered=False)
    return render(
        request,
...
def child_timetable(request, student_code):
    ...
    **Context**
    ``child``
        An instance of :model:`attendane.Student`
    ``timetablevaalues``
    ...
    **Template:**
    ...
    if child.group == 1:
        ...
        academicrecordsA = Timetable.objects.filter(
            subject_name__subject_name__in=academicgroupA)
        for record in academicrecordsA:
            # Avoids error when passing into next url(Will not take an index
            # number starting wiht 0, ie 01)
            if record.day == 0:
        ...
        academicrecordsB = Timetable.objects.filter(
            subject_name__subject_name__in=academicgroupB)
    ...
    if child.music_option == 5:
        ...
        record = get_object_or_404(
            Timetable, subject_name__subject_name="Guitar B")
        if record.day == 0:
            key = f"M{record.session}"
        else:
            key = f"{record.day}{record.session}"
...
def report_absence(request, student_code, session_id):
    ...
    **Context**
    ``child``
        An instance of :modelL`attendance.Student`
    ``session``
        An instance of :model:`attendance.Timetable`
    ``report``
            ...
            if dayofweek == session.day:
                ...
                if DailyRegister.objects.filter(
                    ...
                    messages.warning(
                        ...
                        "Please access it with planned absences button.")
                ...
                messages.error(
                    request,
                    "The date specified does not match "
                    "a timetable slot!"
                    )
                return redirect('reportabsence', student_code=student_code,
                                session_id=session_id)
    report = AbsenceForm()
    return render(
        request,
...
def planned_absences(request, student_code):
    """
    Display a list of instances of :model:`attendance.DailyRegister`
    **Context:""
    ``child``
        An instance of :model:`attendance.Student`
    ``absences``
        A queryset of :model:`attendance.DailyRegister`
    **Template:**
    :template:`attendance.planned.absences.html`
    """
    today = date.today()
    child = get_object_or_404(Student, student_code=student_code)
    absences = DailyRegister.objects.filter(
        student_code__student_code=student_code, date__gt=today)
    return render(
        request,
...
def plan_detail(request, student_code, date, session_id):
    """
    Allows parent to  update or delete a planned absence
    Displays an instance of :model:`attendance.DailyRegister`
    **Context:**
    ``child``
        An instance of :model:`attendance.Student`
    ``absence``
        An insstance og :model:`attendance.DailyRegister`
    ``reasonform``
        An instance of :form:`attendance.ReasonForm
    **Template:**
    :template:`attendance.plan_detail.html`
    """
    child = get_object_or_404(Student, student_code=student_code)
    absence = get_object_or_404(
        DailyRegister,
...
def child_record(request, student_code):
    ...
    **Context**
    ...
    ``childrecords``
        queryset with all the instances of :model:`attendance.DailyRegister`
    ...
    **Template:**
    ...
    child_records = DailyRegister.objects.filter(
        student_code__student_code=student_code)
